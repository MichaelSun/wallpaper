...
public class QRCodeGenerator {
    ...
    private Bitmap composeBitmap(Bitmap topBitmap, Bitmap bottomBitmap,
                                 QRCodeOptions.ComposeType composeType, int leftPadding, int topPadding,
                                 int backgroundColor) {
    ...
    public Bitmap generate(QRCodeOptions options) throws WriterException {
        ...
        if (options.outComposeType != QRCodeOptions.ComposeType.ALTERNATIVE) {
        ...
        if (options.outShape == QRCodeOptions.Shape.WATER) {
        ...
        for (int outputY = topPadding; outputY < outputHeight - topPadding; outputY += boxSize) {
            ...
            for (int outputX = leftPadding; outputX < outputWidth - leftPadding; outputX += boxSize) {
                ...
                if (options.outBackgroundImage != null
                        && options.outComposeType == QRCodeOptions.ComposeType.ALTERNATIVE) {
                    ...
                    } else if (options.outGradientType == QRCodeOptions.GradientType.NORMAL
                    ...
                    QRCodeOptions.Shape shape = options.outShape;
                    if (isSet(input, inputX, inputY)) {
                        if (shape == QRCodeOptions.Shape.ROUND) { // 圆角
                            if (hasTextContent) {
                        ...
                        } else if (shape == QRCodeOptions.Shape.WATER) { // 液态
                            if (hasTextContent) {
                                Rect searchRect = new Rect(outputX, outputY, outputX + boxSize, outputY + boxSize);
                                int color = QRCodeUtil.getColorOnBitmap(textBt, searchRect, textDefaultColor);
                                isTextPoint = (color == textDefaultColor);
                            ...
                            if (!isTextPoint)
                                canvas.drawRect(outputX, outputY, outputX + boxSize, outputY + boxSize, paint);
                        }
                    } else {
                        if (shape == QRCodeOptions.Shape.WATER) {
                            if (hasTextContent) {
                                Rect searchRect = new Rect(outputX, outputY, outputX + boxSize, outputY + boxSize);
                                int color = QRCodeUtil.getColorOnBitmap(textBt, searchRect, textDefaultColor);
                                isTextPoint = (color == textDefaultColor);
